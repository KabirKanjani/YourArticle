...
<Box mt={2} position="relative" width="100%" height="8px" borderRadius={1} bgcolor="#e4e8ef">
  <Box width={`${percentage}%`} bgcolor="#6C5DD3" position="absolute" height="100%" borderRadius={1}/>
</Box>
  </Box>
)
const ArticlesReferralsInfo = [
  {
    title: 'Social Media',
    percentage: 64,
    color: '#6C5DD3',
  ...
  const ArticlesReferrals = () => 
  {
   return (
    <Box mt={2} p={4} flex={1} bgcolor="#fcfcfc" id="chart" display="flex" flexDirection="column" borderRadius="15px">
      ...
      {ArticlesReferralsInfo.map((bar)=> <ProgressBar key={bar.title} {...bar}/>)}
 ...
 export default ArticlesReferrals